...
- **CORS**: `Cors:AllowedOrigins` config array (defaults to localhost:4200 for dev); set via `Cors__AllowedOrigins__0/1/2` Container App env vars. CORS is handled exclusively by ASP.NET Core middleware (not Container Apps ingress) so `AllowAnyHeader()` covers SignalR's `x-signalr-user-agent`. Hub endpoint has explicit `.RequireCors("AppCors")`
...
## Recent Changes
- Issue #45 "Full Site Edit 1" (2026-04-30): UI/UX fixes across 8 pages. **Main page**: classic NHL shield logo (1946-2005 SVG from Wikimedia) replaces text circle. **Standings**: clinch indicator badges (x/y/z/p/e) via `ClinchIndicator` field on `StandingsSnapshot` parsed from NHL API `clinchIndicator`; improved text visibility; themed scrollbar. **Game Hub**: penalty/goal event descriptions now resolve player names from DB via `BuildEvents()` method in `GameHubQueryService` instead of raw `descKey`; no more duplicate penalty text. **Teams**: division rank in parentheses, league rank next to record, clinch badge on profile; roster sorted by jersey number. **Stats**: rookie filtering uses `PlayerSeason.IsRookie` (from NHL API `rookieFlag`) instead of broken `GamesPlayed<=25` heuristic; sticky column headers on scroll. **Scores**: expanded box layout reworked (box scores stacked vertically, stats to the right); expanded spans 3 columns with small boxes beside it; playoff series status (`Game.SeriesStatus`) parsed from NHL API `seriesStatus` and shown in score box. **Schedule**: converted from day-card list to 7-column calendar grid with today highlight. **Draft**: lottery odds column in Round 1 (standard NHL percentages for bottom 16 teams). Three new EF migrations: `AddClinchIndicator`, `AddIsRookieToPlayerSeason`, `AddSeriesStatusToGame`. Component style budget raised from 4kB to 6kB.
- Smoke test bug fixes (2026-04-28): 14 of 15 issues from the 2026-04-19 smoke test fixed. SignalR CORS (moved from Container Apps ingress to ASP.NET Core middleware with explicit `RequireCors` on hub). Stale game reconciliation (`ReconcileStaleGamesAsync` in ScoresSyncJob, ScheduleSyncJob now updates existing games). Dark mode (ThemeService injected in root component). Roster draft data (draft fields mapped during seeding). Game Hub boxscore parsing rewrite (player stats from forwards/defense/goalies, team stats from direct properties, powerPlay "x/y" parsing). Stats realtime merge (`skater/realtime` fetched and merged for HIT/BLK/GV/TK). Franchise data (static founding year + Cup count for all 32 teams). Pregame leaders + season H2H queries implemented. Season mode transition logic fixed (no longer stuck on `regular-season` past `regularSeasonEnd`). Standings labels shortened, draft heading conditional, search player results disabled with notice, placeholder pages replaced with data-unavailable notices. Angular packages updated to 21.2.9 (GHSA-45q2-gjvg-7973 fix). Redis container CPU bumped to 0.25 (Container Apps validation). Only #03 (nav gating on season-mode flags) remains open.
...
- _~~`GetSkaterStatsAsync` HIT/BLK/GV/TK always null~~ — Fixed 2026-04-28. `skater/summary` endpoint doesn't include these fields. Now fetches `skater/realtime` report and merges by (playerId, teamAbbrev)._
- _~~`ParseGameDetail` PlayerStats always empty, team stats all zeros~~ — Fixed 2026-04-28. Implemented player stats parsing from boxscore forwards/defense/goalies arrays. Rewrote `ParseTeamStats` to read direct properties instead of non-existent `teamGameStats` array. PowerPlay parsed as "goals/opps" string._
- _~~`ScoresSyncJob` past games stuck as Live/Scheduled~~ — Fixed 2026-04-28. Added `ReconcileStaleGamesAsync` — sweeps any Live/Scheduled/Pre-Game game with start >6h ago, re-fetches from NHL API, updates status + detail stats. `ScheduleSyncJob` also now updates existing game status._
- **`GetTradesAsync` returns empty** — NHL Web API has no trades endpoint. Stub implementation with log warning. Frontend shows "data not yet available" notice. Needs alternate data source.
