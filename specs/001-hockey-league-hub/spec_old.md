...
- Q: How frequently should live game scores update during active games? → A: Every 60 seconds, automatically without page refresh.
...
- Q: Should the site provide a global search capability across all entities? → A: Yes, a global search bar in the black banner that searches across all entities (players, teams, trades, staff, etc.).
...
- Q: What should the hamburger menu contain in this phase beyond the dark mode toggle? → A: Dark mode toggle and a league selector (preparing for future multi-league expansion).
- Q: How frequently should non-live data (standings, stats, rosters, trades, free agents) refresh from the data source? → A: Standings and rosters refresh hourly; stats, trades, and free agents refresh every 3 hours.
- Q: How far back should historical data extend for player career stats, trade history, and team records? → A: Since the 2000-01 season (25 years of data, covering most active player careers and the modern analytics era).
- Q: How should video content be delivered when a visitor clicks to watch a play? → A: Embed video from the data provider (e.g., NHL.com or Sportradar embed URLs) in a modal player on-site.
...
4. **Given** multiple games on the same day, **When** the scores page loads, **Then** games appear in order of start time, with ties broken by alphabetical order of the home team's city name.
5. **Given** a score box is displayed, **When** the visitor clicks to expand it, **Then** it expands to fill 3 columns by 2 rows, showing period box scores (goals and shots), PP fractions, HIT, FO%, TK, GV, and goal/penalty summaries. Any previously expanded box collapses.
6. **Given** a game has not started, **When** the score box is displayed, **Then** the start time appears where scores would normally be shown, adjusted to the visitor's time zone.
...
1. **Given** a visitor on the Game Hub Team Stats tab, **When** they view the layout, **Then** the box score by period is at the top, the away team's abbreviated name is on the left and the home team's on the right, with a central column of stats (SOG, HIT, PP, FO, GV, TK, TOI) between them, goal summaries to the left (away) and right (home) of the stat column, and penalty summaries beneath each team's goals.
2. **Given** a goal summary entry, **When** displayed, **Then** it shows the period and game clock time, scorer (first initial and last name) with season goal number, and assisting players with season assist numbers.
...
6. **Given** the Player Stats tab, **When** the grids load, **Then** two side-by-side grids appear (away on left, home on right) with skater rows showing #, G, A, PTS, +/-, HIT, PIM, TOI, SOG, FO%, GV, TK, WAR, xGF/60. Goalie rows have a separate header showing SA, SV, SV%, TOI, HDC/HDS, LDC/LDS, WAR. Rows alternate in shading and the grid scrolls horizontally if needed.
7. **Given** the Rink Instances tab, **When** displayed, **Then** an accurately dimensioned rink diagram shows rink measurements, and indicates which side has the home bench and which has the away bench.
...
10. **Given** a visitor clicks a dot that has corresponding video, **When** they click, **Then** a video player pops up with footage of the play.
...
1. **Given** the Standings page loads, **When** the default view appears, **Then** teams are displayed in wild card format, grouped by division and conference, ranked by point percentage, with alternating row shading.
2. **Given** the Standings page, **When** displayed, **Then** the following stats are shown for each team: GP, W, L, OTL, PTS, PTS%, RW, ROW, GF, GA, DIFF, PP%, PK%.
...
1. **Given** a visitor on the Teams index page, **When** displayed, **Then** all current NHL team icons are listed alphabetically by city name.
...
3. **Given** the team profile roster view, **When** displayed, **Then** each player lists their number, position, stick handedness, date of birth, draft year, and years in the league.
...
4. **Given** a trade detail page, **When** a trade tree exists, **Then** it displays a clean, navigable visualization of all subsequent transactions involving the original traded pieces, with each branch going as deep as necessary.
...
- **Unavailable video**: When video is unavailable for a particular play, the video link/option is not shown; only the text summary is displayed.
- **All-Star break and bye weeks**: The calendar reflects days with no games, and the scores page shows a contextual message.
...
- **Goalie playing as a skater**: If a goalie records goals/assists, those appear in the goalie stat line under the G and A columns.
...
- **FR-005**: The site MUST display a black banner at the top of every page with the site name "To Be Determined" on the left, a global search bar in the center, and a hamburger menu button on the right.
- **FR-005a**: The global search bar MUST search across all entity types (players, teams, trades, staff, etc.) and display results grouped by entity type with clickable links to the relevant pages.
- **FR-006**: The site name in the banner MUST be clickable from any page to return to the main page.
- **FR-006a**: A persistent secondary navigation bar MUST appear below the black banner on all league pages, providing direct links to each section: Scores, Standings, Stats, Teams, Players, Salary Cap, Trades, Free Agents, and Personnel. The active section MUST be visually highlighted.
...
- **FR-011d**: Standings and roster data MUST refresh from the data source at least once per hour. Stats, trades, and free agent data MUST refresh at least once every 3 hours. Live game scores continue to refresh every 60 seconds per FR-016/SC-016.
...
- **FR-016**: Game score boxes MUST display in a 4-column grid, ordered by start time with ties broken by alphabetical order of the home team's city.
...
- **FR-021**: Score boxes MUST be expandable to a size of 3 columns by 2 rows, showing period-by-period box scores for goals and shots, PP fractions, HIT, FO%, TK, GV, and goal/penalty summaries for each team.
- **FR-022**: Only one expanded score box may be visible at a time; expanding a new box MUST collapse the previously expanded one.

#### Game Hub
- **FR-023**: The Game Hub MUST provide tabs for Team Stats (default), Player Stats, Lineups, and Rink Instances.
- **FR-024**: The Team Stats tab MUST display the score and box score by period at the top, with abbreviated team names (away on left, home on right) split by a central column of stats: SOG, HIT, PP, FO, GV, TK, and TOI.
- **FR-025**: Goal summaries MUST appear to the left (away team) and right (home team) of the central stats, showing period/time, scorer (first initial and last name) with season goal number, and assists with season assist numbers.
- **FR-026**: Penalty summaries MUST appear beneath each team's goal summaries, showing period/time, offending player, penalty type, and the player's season penalty minute total.
...
- **FR-031**: The Player Stats tab MUST display two side-by-side grids (away left, home right) with skater stats: #, G, A, PTS, +/-, HIT, PIM, TOI, SOG, FO%, GV, TK, WAR, xGF/60.
...
- **FR-034**: The Rink Instances tab MUST display an accurately dimensioned diagram of the game's rink, including measurements and labels for the home bench and away bench sides.
...
- **FR-037**: Hovering over a rink dot MUST display the action type, player, and game clock time; clicking a dot with available video MUST open a modal containing an embedded video player (sourced from the data provider) with footage of the play.

#### Standings
- **FR-038**: The Standings page MUST default to wild card format, grouped by division and conference, ranked by point percentage.
- **FR-039**: Displayed stats MUST include GP, W, L, OTL, PTS, PTS%, RW, ROW, GF, GA, DIFF, PP%, and PK%.
...
#### Teams
- **FR-046**: The Teams index page MUST display all current NHL team icons sorted alphabetically by city name.
...
- **FR-049**: The team roster MUST display each player's number, position, stick handedness, date of birth, draft year, and years in the league.
...
- **FR-069**: Clicking a trade MUST display all known details and a clean, navigable visual trade tree showing all subsequent transactions involving the original pieces, traced as deep as necessary.
...
- **Team**: An organization within a league. Has a roster, depth chart, salary cap allocation, franchise history (including relocation history), personnel, and trade history. Identified by city, name, 3-letter abbreviation, logo, and primary color.
- **Player**: An individual athlete. Has biographical info, career stats across multiple leagues, headshots (current and historical), playing style profile, contract history, trade history, and free agency history. Identified by name, headshot, and jersey number.
- **Game**: A single contest between two teams on a specific date at a specific venue. Contains scores by period, play-by-play events (goals, penalties, shots, hits, giveaways, takeaways), player stats, and an official score sheet. Events include location coordinates for rink diagrams.
- **Trade**: An exchange of players, draft picks, or other assets between teams. Forms trade trees through subsequent transactions involving the traded pieces. Has a date and the participating teams.
...
- **Rink/Arena**: A physical venue with specific ice surface dimensions and bench locations. Used for accurate rink instance diagrams.
...
- **SC-011**: Rink diagrams accurately reflect each venue's dimensions and correctly identify home and away bench sides.
- **SC-012**: Trade trees display the complete chain of transactions with no missing links for trades where data is available.
...
- **SC-016**: During live games, scores on the scores page and Game Hub update automatically every 60 seconds without requiring a page refresh.

## Assumptions

- The initial release covers the NHL only; other leagues (AHL, ECHL, KHL, SHL, Liiga, etc.) will be added in future phases.
- Account creation, user authentication, personalization, and custom settings are deferred to a future phase. The hamburger menu will contain a dark mode toggle and a league selector (currently showing only NHL, but structured for future multi-league expansion).
...
- The primary data source for game data, player stats, event coordinates, and video is the NHL official API or a licensed data aggregator (e.g., Sportradar). Video content is delivered by embedding the provider's video URLs in a modal player on-site; no video is self-hosted. Video content availability depends on the licensing agreement with the chosen provider; plays without available video simply do not display a video option.
...
- Pre-2000 historical data (stats, trades, records) is deferred to a future phase.
- A schedule view will be accessible from the scores page calendar date picker, showing an abbreviated season schedule.
- Scores update automatically every 60 seconds during live games without requiring manual page refresh. Non-live data follows a tiered refresh cadence: standings and rosters hourly, stats/trades/free agents every 3 hours.
- Rink diagrams are based on publicly available arena dimension data for each NHL venue.
- The site is delivered as a statically generated site with incremental rebuilds triggered at each data refresh interval (60s for live scores, hourly for standings/rosters, every 3 hours for stats/trades/free agents), served via CDN. Client-side polling handles live score updates between rebuilds.
...
- Historical data covers the 2000-01 season through the present. Player career stats, trade history, team records, and all-time records are scoped to this range. Pre-2000 data is out of scope for the initial release.
...
### In Scope
- All primary sections: Main Page, Scores, Game Hub, Standings, Stats, Teams, Players, Salary Cap, Trades, Free Agents, Personnel
...
- Expandable score boxes with exclusive open behavior
- Sortable stat columns across standings, stats, and free agent pages
- Rink event diagrams with team-colored dots and five switchable views
- Trade trees with full transaction chain visualization
