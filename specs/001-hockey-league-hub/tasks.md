...
- [ ] M001 [P] Create global shell mockup (black banner with site name + search bar + hamburger icon, secondary nav bar with all 11 sections, live score ticker bar between banner and nav, hamburger menu flyout with Account/Settings/League Selector/dark mode toggle) showing both light and dark modes in `docs/mockups/01-global-shell.html`
- [ ] M002 [P] Create scores page mockup (4-column score box grid with: collapsed boxes showing away-above-home/scores-right/SHG/record/rank, one expanded box showing period breakdowns + PP/HIT/FO%/TK/GV + goal/penalty summaries, one unstarted-game box with pregame matchup summary showing top scorers/goalies/PP%/PK%/H2H, calendar date picker, Game Hub link) in `docs/mockups/02-scores-page.html`
...
- [ ] T025 Create secondary navigation bar component (Scores, Standings, Stats, Teams, Schedule, Players, Salary Cap, Trades, Free Agents, Personnel) in `frontend/src/app/components/layout/nav-bar/`
...
- [ ] T028 Create live score ticker component shell (between banner and nav bar, subscribes to SignalR) in `frontend/src/app/components/layout/score-ticker/`
...
- [ ] T048 [US1] Create expanded score box component (period breakdowns, PP, HIT, FO%, TK, GV, goal/penalty summaries) in `frontend/src/app/components/scores/expanded-score-box/`
- [ ] T049 [US1] Create pregame matchup component (top scorers, starting goalies with GAA/SV%, PP%/PK%, head-to-head) in `frontend/src/app/components/scores/pregame-matchup/`
- [ ] T050 [US1] Implement live score ticker with real-time game clock countdown (team icons, score, period, clock) in `frontend/src/app/components/layout/score-ticker/`
