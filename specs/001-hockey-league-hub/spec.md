...
5. **Given** a score box is displayed, **When** the visitor clicks to expand it, **Then** a single enlarged box replaces the collapsed box (never showing both), displaying: period-by-period goals box score, period-by-period shots on goal box score, a vertical stat column (PP, HIT, FO%, TK, GV, TOP), away team goal/penalty summaries to the left of the stats, and home team goal/penalty summaries to the right of the stats. Any previously expanded box collapses. Other games' small boxes flow beside the expanded box before wrapping to a new row.
...
- **FR-006b-i**: All game times, scoreboard times, and "Final" status labels in the score bar and score boxes MUST be displayed in white with the same font weight as team abbreviations. Game clocks showing 5 minutes or fewer remaining in games where the score difference is 2 goals or fewer MUST be displayed in red.
...
- **FR-021**: Score boxes MUST be expandable. The expanded view MUST include: (a) a period-by-period goals box score, (b) a period-by-period shots on goal box score below the goals box score, (c) a vertical stat column containing PP, HIT, FO%, TK, GV, and TOP (time of possession), (d) away team goal summaries and penalty summaries to the LEFT of the stat column, (e) home team goal summaries and penalty summaries to the RIGHT of the stat column. If the expanded box would be too cramped with the original inline scoreboard, the scoreboard MAY be removed from the expanded view; in that case, each team's season record and league standings rank MUST be shown next to the team name in the box score table.
- **FR-021a**: Score boxes for games that have not started MUST appear in collapsed state using the same compact format as active games — showing the start time and basic team info. When expanded, the pregame box MUST show a matchup summary including: top goal scorers per team, top points getters per team, starting goalie GAA and SV% (or "Awaiting starting goalie confirmation"), each team's PP% and PK%, and head-to-head record. The head-to-head record MUST show each team's number of wins, overtime wins, and shootout wins against the other for the current season, and each team's number of wins, overtime wins, shootout wins, and ties against the other all time, displayed in a clean stat-type format.
- **FR-022**: Only one expanded score box may be visible at a time; expanding a new box MUST collapse the previously expanded one. Only one box per game MUST exist (either small or large, never both simultaneously). When a box is expanded, other games' small boxes MUST flow beside the expanded box (left or right) as long as they fit, only wrapping to a new row when space runs out.
