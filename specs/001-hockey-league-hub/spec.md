...
- Q: How should the stats table in the expanded score box be laid out? → A: Condense the stats table to half its current width. Place the head-to-head Season and All-Time record tables adjacent to it (side by side) to save room.
- Q: What player stats should the pregame expanded box show alongside goalie stats? → A: Along with the starting goalie stats, show the top goal scorer, top assist maker, and top points getter on each team. If the same player leads in more than one category, list them for each stat they lead.
...
1. **Given** a visitor on the Game Hub Team Stats tab, **When** they view the layout, **Then** a goals-by-period box score and a shots-by-period box score are displayed side by side at half width each at the top, with the Team Comparison (SOG, HIT, PP, FO, GV, TK, TOI showing away on left and home on right) placed adjacent to the right of the box scores. Goal summaries and penalty summaries appear together in a single card below, separated by a sharp horizontal dividing line, with away team summaries on the left and home team summaries on the right. Vertical spacing between sections is minimized to reduce wasted space.
