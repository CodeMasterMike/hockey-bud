...
5. **Given** a score box is displayed, **When** the visitor clicks to expand it, **Then** it expands to fill 3 columns by 2 rows, showing period box scores (goals and shots), PP fractions, HIT, FO%, TK, GV, and goal/penalty summaries for each team. Any previously expanded box collapses.
6. **Given** a game has not started, **When** the score box is displayed, **Then** the start time appears where scores would normally be shown, adjusted to the user's detected time zone. The visitor can expand the box to view a pregame matchup summary including: the stats of the top goal scorers on each team, the top points getters of each team, the GAA and SV% of the starting goalies (if known; if not, indicate "Awaiting starting goalie confirmation"), each team's PP% and PK%, and the head-to-head matchup record for the current season and all time.
...
- **FR-006a**: A persistent secondary navigation bar MUST appear below the black banner on all league pages, providing direct links to each section: Scores, Standings, Stats, Teams, Schedule, Players, Salary Cap, Trades, Free Agents, and Personnel. The active section MUST be visually highlighted.
- **FR-006b**: An abbreviated live score ticker MUST appear between the top black banner and the secondary navigation tabs on all league pages. It MUST display only the team icons, current score, period, and game clock (counting down in real time with the actual game). This ticker is visible on every page within a league during active games.
...
- **FR-021a**: Score boxes for games that have not started MUST be expandable to show a pregame matchup summary including: top goal scorers per team, top points getters per team, starting goalie GAA and SV% (or "Awaiting starting goalie confirmation"), each team's PP% and PK%, and head-to-head record for the current season and all time.
...
### In Scope
- All primary sections: Main Page, Scores, Standings, Stats, Teams, Schedule, Players, Salary Cap, Trades, Free Agents, Personnel
